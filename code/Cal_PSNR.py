...
def main():
    ...
    folder_GT = r'../benchmark/Test/X4/HR'
    folder_Gen = r'../save/ITNSR/Test/Testx4-Results'
  
    filepath = '../save/ITNSR/Test/Testx4-Results/index_X4.csv'

    
    crop_border = True
    suffix = '_x4'  # suffix for Gen images
    ...
    for i, img_path in enumerate(img_list):
        ...
        write_csvlog(filepath, {'name': base_name, 
                               'PSNR': PSNR,
                               'SSIM': SSIM})
...
def write_csvlog(filepath, values: dict):
    log = filepath
    fieldnames = list(values.keys())
    open_type = 'a' if os.path.exists(log)else 'w'
    with open(log, open_type) as fp:
